...
const app        = express();
const PORT       = process.env.PORT || 4000; // Render assigns its own PORT
...
// screenshot) — lives in its own Mongo document so editing it never
// touches the master shift roster above ──────────────────────
const SEED_TRACKER_ROSTER = {
  people: [
    { id: 'r1',  name: 'Arun R',             region: 'ANZ/APAC',   mode: 'SMB' },
    { id: 'r2',  name: 'Gokulakrishnan P',   region: 'ANZ/APAC',   mode: 'SMB' },
    { id: 'r3',  name: 'Premnath V',         region: 'APAC/India', mode: 'SMB' },
    { id: 'r4',  name: 'Tamilselvan G',      region: 'APAC/India', mode: 'SMB' },
    { id: 'r5',  name: 'Subbu',              region: 'India/MEA',  mode: 'EBS' },
    { id: 'r6',  name: 'Manoj Kumar',        region: 'India/MEA',  mode: 'SMB' },
    { id: 'r7',  name: 'Prasanth Ganesan',   region: 'India/MEA',  mode: 'SMB' },
    { id: 'r8',  name: 'Anto',               region: 'India/MEA',  mode: 'EBS' },
    { id: 'r9',  name: 'Kewin',              region: 'MEA/Europe', mode: 'EBS' },
    { id: 'r10', name: 'Santhoshraj V',      region: 'MEA/Europe', mode: 'SMB' },
    { id: 'r11', name: 'Sabrishwaran',       region: 'UK/Europe',  mode: 'SMB' },
    { id: 'r12', name: 'Vinoth Kumar',       region: 'UK/Europe',  mode: 'EBS' },
    { id: 'r13', name: 'Arunkumar E',        region: 'UK/Europe',  mode: 'SMB' },
    { id: 'r14', name: 'Prabhakaran K',      region: 'US/LATAM',   mode: 'SMB' },
    { id: 'r15', name: 'Lokesh',             region: 'US/LATAM',   mode: 'EBS' },
    { id: 'r16', name: 'Rahul Muthiah V',    region: 'US/LATAM',   mode: 'SMB' },
    { id: 'r17', name: 'Abinaya K',          region: 'US/LATAM',   mode: 'SMB' },
    { id: 'r18', name: 'Guna',               region: 'US/LATAM',   mode: 'EBS' },
    { id: 'r19', name: 'Santhosh L',         region: 'US/LATAM',   mode: 'SMB' },
    { id: 'r20', name: 'Ranjith Kumar R',    region: 'US/LATAM',   mode: 'SMB' }
  ]
};
...
async function seedTrackerRosterIfNeeded() {
  if (!(await db.trackerRosterExists())) {
    await db.setTrackerRoster(SEED_TRACKER_ROSTER);
    console.log('[tracker] Seeded initial tracker roster into MongoDB.');
  }
}
async function ensureEbsAccounts() {
...
async function scheduleExists(date) { return await db.scheduleExists(date); }
async function readSavedSchedule(date) {
  try { return await db.getSchedule(date); } catch (e) { return null; }
}
async function writeSchedule(date, rows) {
  await db.setSchedule(date, rows);
}
...
async function readRoster() {
  try { return await db.getRoster(); }
  catch (e) { return { cycleStart: '05:00', people: [] }; }
}
async function writeRoster(roster) {
  await db.setRoster(roster);
}
async function seedRosterIfNeeded() {
  if (!(await db.rosterExists())) {
    await db.setRoster(SEED_ROSTER);
    console.log('[master-roster] Seeded initial roster into MongoDB.');
  }
}
...
async function readTrackerRoster() {
  try { return await db.getTrackerRoster(); }
  catch (e) { return { people: [] }; }
}
async function writeTrackerRoster(roster) {
  await db.setTrackerRoster(roster);
}
async function readTrackerDay(date) {
  try { return await db.getTrackerDay(date); } catch (e) { return { events: [] }; }
}
async function writeTrackerDay(date, day) {
  await db.setTrackerDay(date, day);
}
...
async function defaultDaySequence() {
...
async function getOrBuildDayRows(date) {
  const saved = await readSavedSchedule(date);
  if (saved) {
    const { rows: fixed, changed } = backfillDurations(saved);
    if (changed) await writeSchedule(date, fixed); // one-time, on-read migration
    return fixed;
  }
  const roster = await readRoster();
  return computeSchedule(roster.cycleStart, await defaultDaySequence());
}
...
app.get('/api/login/ebs-list', async (req, res) => {
  const users = await readUsers();
...
app.post('/api/login/ebs', async (req, res) => {
...
app.get('/api/schedule/:date', requireAuth, async (req, res) => {
  ...
  res.json({ success: true, date, rows: await getOrBuildDayRows(date), saved: await scheduleExists(date) });
});

// ── Toggle a single person's leave status for a date ──────────
app.post('/api/schedule/:date/leave', requireAuth, requireAdmin, async (req, res) => {
  const { date } = req.params;
  ...
  let dayRows = (await getOrBuildDayRows(date)).map(r => ({ id: r.id, name: r.name, region: r.region, onLeave: !!r.onLeave }));
  ...
  await writeSchedule(date, computed);
...
app.post('/api/schedule/:date/reorder', requireAuth, requireAdmin, async (req, res) => {
  const { date } = req.params;
  ...
  const current = await getOrBuildDayRows(date);
  const byId = {};
  current.forEach(r => { byId[r.id] = { id: r.id, name: r.name, region: r.region, onLeave: !!r.onLeave }; });
...
app.post('/api/schedule/:date/regenerate', requireAuth, requireAdmin, async (req, res) => {
  const { date } = req.params;
  if (!isValidDate(date)) return res.status(400).json({ success: false, message: 'Invalid date format' });
  const roster = await readRoster();
  const computed = computeSchedule(roster.cycleStart, await defaultDaySequence());
  await writeSchedule(date, computed);
...
// ── Master roster CRUD ─────────────────────────────────────────
app.get('/api/master-roster', requireAuth, async (req, res) => {
  res.json({ success: true, ...(await readRoster()) });
});

app.put('/api/master-roster', requireAuth, requireAdmin, async (req, res) => {
...
app.post('/api/master-roster/import', requireAuth, requireAdmin, upload.single('file'), async (req, res) => {
...
app.put('/api/tracker/roster', requireAuth, requireAdmin, async (req, res) => {
  ...
  await writeTrackerRoster({ people: cleanPeople });
...
app.get('/api/tracker/:date', requireAuth, async (req, res) => {
  const { date } = req.params;
  if (!isValidDate(date)) return res.status(400).json({ success: false, message: 'Invalid date format, expected YYYY-MM-DD' });
  res.json({ success: true, date, ...summarizeTrackerDay(await readTrackerDay(date)) });
});

// Click "+" on an SMB tech — logs which EBS tech assigned them a chat
app.post('/api/tracker/:date/assign', requireAuth, requireAssignAccess, async (req, res) => {
  const { date } = req.params;
  const { smbId } = req.body || {};
  if (!isValidDate(date)) return res.status(400).json({ success: false, message: 'Invalid date format' });
  ...
  await writeTrackerDay(date, day);
  console.log(`[tracker] ${date} — ${ebsPerson.name} assigned a chat to ${smbPerson.name}`);
  res.json({ success: true, date, ...summarizeTrackerDay(day) });
});

// Undo the most recent assignment for the day (mis-click correction)
app.post('/api/tracker/:date/undo', requireAuth, requireAdmin, async (req, res) => {
  const { date } = req.params;
  if (!isValidDate(date)) return res.status(400).json({ success: false, message: 'Invalid date format' });
  const day = await readTrackerDay(date);
...
app.post('/api/tracker/:date/reset', requireAuth, requireAdmin, async (req, res) => {
  const { date } = req.params;
  if (!isValidDate(date)) return res.status(400).json({ success: false, message: 'Invalid date format' });
  await writeTrackerDay(date, { events: [] });
...
app.get('/api/export/leave/day/:date', requireAuth, async (req, res) => {
  const { date } = req.params;
  if (!isValidDate(date)) return res.status(400).json({ success: false, message: 'Invalid date format, expected YYYY-MM-DD' });

  const rows = (await getOrBuildDayRows(date)).filter(r => r.onLeave);
...
app.get('/api/export/leave/month/:month', requireAuth, async (req, res) => {
...
app.get('/api/dates-with-data', requireAuth, async (req, res) => {
  try {
    const dates = await db.listScheduleDates();
...
async function runReminderCheck() {
  ...
  const rows = await getOrBuildDayRows(today);
...
// ── Start server: connect to MongoDB first, seed data if needed,
// THEN start listening (so no request arrives before the DB is ready) ─
async function main() {
  await db.connect();
  await seedRosterIfNeeded();
  await seedUsersIfNeeded();
  await seedTrackerRosterIfNeeded();
  await ensureEbsAccounts();
  ...
  app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log('║  Data:    MongoDB Atlas (ziashift database)   ║');
    console.log('╚══════════════════════════════════════════════╝');
    console.log('');
